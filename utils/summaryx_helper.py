    ...
    def addGroup(self, *, scalar=[], tensor=[], image=[], text=[], embedding=[], prefix=""):
        ...
        def write(i, data):
            if self.writer is None:
                ...
                self.writer.add_text("args", json.dumps(
                    self.args, indent=2).replace("\n", "  \n\n"), i)

            for name in scalar:
                if name in data:
                    self.writer.add_scalar(
                        "%s/%s" % (prefix, name), data[name], i)
            for name in tensor:
                if name in data:
            ...
            for name in image:
                if name in data:
                    self.writer.add_image("%s/%s" %
                                          (prefix, name), data[name], i)
            for name in text:
                if name in data:
                    markdown_text = data[name].replace("\n", "  \n\n")
                    self.writer.add_text("%s/%s" %
                                         (prefix, name), markdown_text, i)
