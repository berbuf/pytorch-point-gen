...
from utils import zeros, LongTensor, BaseNetwork, MyGRU, Storage, gumbel_max, flattenSequence, SingleAttnGRU, SequenceBatchNorm
...
class PostEncoder(nn.Module):
    def __init__(self, param):
        ...
        self.postGRU = MyGRU(args.embedding_size,
                             args.eh_size, bidirectional=True)
    ...
    def forward(self, incoming):
        ...
        hidden.h_n, hidden.h = self.postGRU.forward(
            incoming.post.embedding, incoming.data.post_length, need_h=True)
        if self.args.batchnorm:
...
class GenNetwork(nn.Module):
    def __init__(self, param):
        ...
        self.GRULayer = SingleAttnGRU(
            args.embedding_size, args.dh_size, args.eh_size * 2, initpara=False)
    ...
    def teacherForcing(self, inp, gen):
        ...
        _, gen.h, _ = self.GRULayer.forward(
            embedding, length-1, inp.post, inp.post_length, h_init=inp.init_h)
