...
class SingleTurnDialogRecorder(MetricBase):
    ...
    def forward(self, data):
        ...
        for i, post_sen in enumerate(post_allvocabs):
            self.post_list.append(
                self.dataloader.convert_ids_to_tokens(post_sen[1:]))
            self.resp_list.append(
                self.dataloader.convert_ids_to_tokens(resp_allvocabs[i][1:]))
    ...
    def close(self):
        ...
        '''
        res = super().close()
        res.update({"post": self.post_list,
                    "resp": self.resp_list, "gen": self.gen_list})
        return res
