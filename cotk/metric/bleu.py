...
class BleuCorpusMetric(MetricBase):
    ...
    def close(self):
        ...
        try:
            result.update({"bleu":
                           corpus_bleu(
                               self.refs, self.hyps, smoothing_function=SmoothingFunction().method3),
...
class SelfBleuCorpusMetric(MetricBase):
    ...
    def close(self):
        ...
        if len(self.hyps) == 1:
            raise RuntimeError(
                "Self-Bleu can't be computed because there is only 1 generated sentence.")
        if self.sample <= 1:
        ...
        tasks = ((ref[:i]+ref[i+1:self.sample], _ref[i])
...
class FwBwBleuCorpusMetric(MetricBase):
    ...
    def close(self):
        ...
        sample_hyps = self.sample if self.sample < len(
            self.hyps) else len(self.hyps)
        sample_refs = self.sample if self.sample < len(
            self.refs) else len(self.refs)
        ...
        self._hash_relevant_data(
            self.refs + [self.seed, sample_hyps, sample_refs])
    ...
    def forward(self, data):
        ...
        for i, turn_length in enumerate(length):
            ...
            for j in range(turn_length):
                ...
                self.refs.append(
                    [list(self.dataloader.trim(ref_session[j])[1:])])
