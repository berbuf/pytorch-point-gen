...
class PerplexityMetric(MetricBase):
    ...
    def __init__(self, dataloader,
        ...
        # can be 'default', 'pytorch' when first forward time
    ...
    def forward(self, data):
        ...
        if self.engine_version == "pytorch":
            ...
            with torch.no_grad():
                self._pytorch_forward(
                    resp_allvocabs, resp_length, gen_log_prob)
        else:
    ...
    def _normal_forward(self, resp_allvocabs, resp_length, gen_log_prob):
        ...
        if resp_length[checkid] < 2:
            raise ValueError(
                "resp_length must no less than 2, because <go> and <eos> are always included.")
        ...
        for i, resp_len in enumerate(resp_length):
            ...
            if self.full_check:
                ...
                if not np.allclose(expsum, [1] * (resp_len - 1)):
                    raise ValueError(
                        "data[gen_log_prob_key] must be processed after log_softmax.")
            ...
            if not self.invalid_vocab:
                resp_known[resp_known >=
                           self.dataloader.vocab_size] = self.dataloader.unk_id

            self.gen_valid_log_prob.append(
                gen_now[list(range(resp_len-1)), resp_known])
            self.gen_unk_log_prob.append(
                gen_now[:resp_len-1, self.dataloader.unk_id])
    ...
    def _pytorch_forward(self, resp_allvocabs, resp_length, gen_log_prob):
        ...
        for i, resp_len in enumerate(resp_length):
            ...
            # calc normal vocab
            normal_mask = ((resp_now != unk_id) & (
                resp_now < vocab_size)).float()
            word_loss = -(gen_now.gather(-1, resp_known.unsqueeze(1))
                          [:, 0] * normal_mask).sum()
            ...
            if self.invalid_vocab:
                extra_invalid_log_prob = gen_now.gather(-1, resp_now.unsqueeze(1))[
                    :, 0] * invalid_mask
    ...
    def _run_f(cls, ele):
        ...
        invalid_log_prob = unk_log_prob[invalid_idx] - \
            np.log(all_vocab_size - vocab_size)
        if invalid_vocab:
...
class MultiTurnPerplexityMetric(MetricBase):
    ...
    def forward(self, data):
        ...
        for i, sent_length in enumerate(length):
            ...
            turn_length = sent_length.index(
                0) if 0 in sent_length else len(sent_length)
