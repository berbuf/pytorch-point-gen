    ...
    def postprocess(self, local_path):
        ...
        for key in ['train', 'test', 'dev']:
            ...
            response_path = os.path.join(
                local_path, 'opensub_pair_%s.response' % key)
    ...
    def postprocess(self, local_path):
        ...
        for key in ['train', 'test', 'dev', 'multi_ref']:
            filepath = os.path.join(
                local_path, 'switchboard_corpus_%s.jsonl' % key)
            ...
            with open(new_filepath, 'w', encoding='utf-8') as fout:
                if key != 'multi_ref':
                    ...
                    # [session1, response1, session2, response2, ...]
                    dataset = chain.from_iterable(zip(sessions, responses))
        ...
        for key in ['train', 'test', 'dev']:
            if not os.path.isfile(os.path.join(local_path, key + '.txt')):
                raise FileNotFoundError(
                    "there isn\'t %s in %s" % (key + '.txt', local_path))
            else:
...
class CNNResourceProcessor(BaseResourceProcessor):
    ...
    def postprocess(self, local_path):
        local_path = super().postprocess(local_path)
        tmp_local_path = os.path.join(local_path, 'tokenized')
        new_local_path = os.path.join(local_path, 'processed')
        os.makedirs(new_local_path, exist_ok=True)
        for key in ['train', 'test', 'dev']:
            dir_path = os.path.join(local_path, key)
...
class GloveResourceProcessor(ResourceProcessor):
    ...
    def basepreprocess(self, local_path, name):
        ...
        for filename in filenames:
            ...
            os.rename(os.path.join(dst_dir, filename),
                      os.path.join(sub_dir, 'glove.txt'))
        return dst_dir
