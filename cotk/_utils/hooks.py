...
def hook_dataloader(fn):
    ...
    def wrapped(*args, **kwargs):
        ...
        self = bound['self']
        del bound['self']
        invoke_listener("add_dataloader", self,
                        fn.__qualname__.split(".")[0], bound)
...
def hook_metric(fn):
    ...
    def wrapped(*args, **kwargs):
        ...
        self = bound['self']
        del bound['self']
        invoke_listener("add_metric", self,
                        fn.__qualname__.split(".")[0], bound)
        return fn(*args, **kwargs)
    return wrapped
...
def hook_wordvec(fn):
    ...
    def wrapped(*args, **kwargs):
        ...
        self = bound['self']
        del bound['self']
        invoke_listener("add_wordvec", self,
                        fn.__qualname__.split(".")[0], bound)
        return fn(*args, **kwargs)
    return wrapped
    ...
    def invoke_metric_close(self, obj, return_dict):
        for key, value in return_dict.items():
            ...
            try:
                ...
                continue
            metric_args = {
                key: value for key, value in self.metric_set[obj].items() if key != "dataloader"}
