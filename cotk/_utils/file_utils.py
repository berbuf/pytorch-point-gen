...
def _get_resource(file_id, cache_dir=CACHE_DIR, config_dir=CONFIG_DIR):
    ...
    if config['type'] != res_type:
        raise ValueError("res_type {} differs with res_type {}".format(
            res_type, config['type']))

    resource_processor = ResourceProcessor.load_class(
        res_type + 'ResourceProcessor')(cache_dir, config_dir)
    if resource_processor is None:
        raise RuntimeError(
            "No resources type named %sResourcePreprocessor" % res_type)
    ...
    if not os.path.exists(meta_path):
        ...
        if content_hash == config['hashtag']:
            ...
            print("bad hashtag {}, correct is {}".format(
                content_hash, config['hashtag']))
...
def _load_local_data(local_path):
    ...
    LOGGER.info('processor type: %s', res_type)
    print(res_type + 'ResourceProcessor')
    resource_processor = ResourceProcessor.load_class(
        res_type + 'ResourceProcessor')()
...
def get_resource_file_path(file_id, cache_dir=CACHE_DIR, config_dir=CONFIG_DIR):
    ...
    if file_id.startswith('resources://'):
        return _get_resource(file_id[12:], cache_dir, config_dir)
    elif file_id.startswith('http://') or file_id.startswith('https://'):
        return _download_data(file_id, cache_dir)
    else:
        return _load_local_data(file_id)
